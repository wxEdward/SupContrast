...
model_names = ["resnet50", "aconvnet"]

model_paths = {"resnet50": "./models/resnet50-checkpoint-110.pt",
               "aconvnet": "./models/aconvnet-checkpoint-150.pt"}

model_loads = {"resnet50": load_resnet50,
...
'''
 ...
 @param notation: the set of pixels belonging to the object region
 @param overlay: DEPRECATED
 @param mag: min and max magnitudes of original SAR image

 @param N: number of scatters
 @param batch: number of attacks running in parallel
...
'''
def attack(model, device, X_image, y_gt, notation, overlay, mag, batch, N, theta_min, theta_max, vth, lambd, lambd_gaussian, n_max, S0):
    assert(mag.shape == (2, ))
    print(mag[0], mag[1])
    assert(mag[0] < mag[1])
    ...
    while iteration < n_max and v.min().item() > vth:
        ...
        # reduce amplitude if a scatter's max pixel value > 1            
        for i in range(N):
...
def all():
    ...
    for i, (image, label) in enumerate(zip(X_test_image, test_label)):
        ...
        param = attack(surrogate_model, device, image, label, test_notation[str(i)], overlays[i], test_mag[i], batch=batch, N=N, theta_min=theta_min, theta_max=theta_max, vth=vth, lambd=lambd, lambd_gaussian=lambd_gaussian, n_max=n_max, S0=S0)
    ...
    np.save('adv_dataset_N1/adv_images_{}'.format(suffix), X_adv_images, allow_pickle=False)
    np.save('adv_dataset_N1/adv_images_obj_{}'.format(suffix), X_adv_filtered_images, allow_pickle=False)
    np.save('adv_dataset_N1/adv_gt_{}'.format(suffix), gt, allow_pickle=False)
    np.savetxt('adv_dataset_N1/params_{}.txt'.format(suffix), params, fmt='%.4f')
    np.savetxt('adv_dataset_N1/params_obj_{}.txt'.format(suffix), params_filtered, fmt='%.4f')
